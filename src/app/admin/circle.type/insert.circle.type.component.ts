...
@Component({
    selector: 'insert-circle-type-com',
    templateUrl: 'insert.circle.type.component.html',
    styleUrls: ['insert.circle.type.component.css']
})
export class InsertCircleTypeComponent {
    dataForm: FormGroup;
    today: Date = new Date();
    ...
    ngOnInit() {
        ...
        this.route.data.pipe(
            map((data) => data['org_circletype']),take(1)).subscribe((org_circletype) => {
                if (org_circletype.length > 0) {
                    this.data_list = org_circletype;
    ...
    createForm() {
        this.dataForm = this.fb.group({
            _id: [''],
            circletype_code: ['', Validators.compose([Validators.required, Validators.pattern('[0-9]*')])],
            circletype_name: ['', Validators.required],
            last_update_long: [''],
            last_update_short: ['']
        });
    }
    //-------------------------------------------------------------------------------
    save_circletype(data) {
        let find_index = _.findIndex(this.data_list, function (o) {
            return o.circletype_code == data.circletype_code || o.circletype_name == data.circletype_name;
        });
        if (find_index != -1) {
            this.resetForm();
            ...
            this._http.save_circletype(data).pipe(take(1)).subscribe((json: any) => {
                if (json.result.n >= 1) {
    ...
    //-------------------------------------------------------------------------------
    update_circletype(data?: any) {
        let list = [...this.data_list];
        data.last_update_short = this.farsiDate_short;
        data.last_update_long = this.farsiDate_long;

        let find_index = _.findIndex(this.data_list, function (o) {
            return o.circletype_name == data.circletype_name || o.circletype_code == data.circletype_code;
        });
        if (find_index != -1 && find_index != this.index) {
            ...
            this._http.update_circletype(data).pipe(take(1)).subscribe((json: any) => {
                if (json.nModified >= 1) {
    ...
    //-------------------------------------------------------------------------------
    export() {
        this._msg.export_to_excel(this.dataSource.data, 'دوایر سازمانی');
    }
    //-------------------------------------------------------------------------------
